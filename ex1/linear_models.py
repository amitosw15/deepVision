...
class LinearClassifier:
    ...
    def train(self,
        ...
        for i in range(num_iters):
            ...
            loss, lossDelta = self.loss(X_batch,y_batch)
            ...
            #                          START OF YOUR CODE                             #                                                         #
            ###########################################################################
            self.W -= learning_rate*lossDelta
...
def softmax_cross_entropy(W: np.ndarray, X: np.ndarray, y: np.ndarray):
    ...
    # 1. Numerical Stabilization: Subtract the maximum score in each row (axis 1).
    max_scores = np.max(scores, axis=1, keepdims=True)
    scores_shift = scores - max_scores
    
    # 2. Compute stabilized exponential
    ...
    #############################################################################
    epsilon = 1e-12
    loss = -np.sum(np.log(correct_class_probabilities+epsilon)) / N
    #############################################################################
    #                            END OF YOUR CODE                               #
    #############################################################################
    #############################################################################
...
def softmax(x: np.ndarray) -> np.ndarray:
    ...
    # The new exponents are guaranteed to be <= 0, preventing overflow.
    exp_x = np.exp(x_shift)
    
    # 3. Compute probabilities
    # Divide the exponents by the sum of exponents for each row.
...
def softmax_cross_entropy_vectorized(W: np.ndarray, X: np.ndarray, y: np.ndarray):
    ...
    # 2. Compute Softmax Probabilities (using the defined softmax function)
    # The softmax function handles the shifting (stabilization) internally.
    ...
    # Add epsilon for numerical stability when taking the log of very small numbers
    epsilon = 1e-12 
    loss = -np.sum(np.log(correct_class_probabilities + epsilon)) / N
    #############################################################################
    # END OF YOUR CODE                                                          #
    ...
    # P is 'probabilities'. T (Target/One-hot) is implicitly created by subtraction.
    dS = probabilities.copy()
    dS[np.arange(N), y] -= 1 # Subtract 1 from the correct class's score gradient
    
    # 2. Calculate dW (dL/dW = X.T @ dL/dS)
    dW = X_use.T @ dS

    # 3. Average the Gradient
    dW /= N
    ...
    *,
    ...
    for batch in batch_sizes:
      for rate in learning_rates:
          perceptron = ModelClass(X_train, y_train, **model_kwargs)
          loss_history = perceptron.train(X_train, y_train, learning_rate=rate,
                                num_iters=num_iters, batch_size=batch, verbose=True)
